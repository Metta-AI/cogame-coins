## Rank normal Coins intents from the player's seat-private observation.
...
proc chooseAction*(observation: JsonNode): JsonNode =
  ...
  for intent in observation["rules"]["intents"]:
    let name = intent.getStr()
    criteria[name] = %name
  ...
  if endpoint.len == 0 or (sidecar.len == 0 and key.len == 0):
    raise newException(ValueError, "Coins Jev policy has no model transport")
  ...
  let body = %*{
    "model": model,
    "state": "You are playing Coins. Maximize your score. This " &
      "seat-private observation is all you may use:\n" & $observation,
    "questions": {"decision": {
      "type": "choice",
      "instructions": "Choose the movement intent for this beat.",
      "criteria": criteria
    }}
  }
