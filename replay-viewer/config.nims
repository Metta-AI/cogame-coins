...
# use-after-free fixed in coins_replay.nim (emscripten_exit_with_live_runtime)
...
# coins_error_len(). Aborting keeps linear memory intact, and the page reads
# coins_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "coins_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_coins_load_replay,_coins_frame,_coins_input,_coins_packet_ptr,_coins_packet_len,_coins_error_ptr,_coins_error_len,_coins_stage_ptr,_coins_stage_len
  """).replace("\n", " ")
)
