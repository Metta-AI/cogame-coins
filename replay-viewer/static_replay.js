(function () {
  ...
  // VIEWER -> HOST READINESS. An embedding page (the softmax.com theater,
  // the Observatory episode page) can only see this document's `load` event,
  // which fires long before the wasm module has compiled and the replay has
  // come back from S3. `ready` is posted from the callback that runs AFTER
  // `data-replay-loaded="true"` is set (the chorus fix — posting on rAF
  // timing at the call site lets softmax.com sample an unpainted shell).
  ...
  function showFailure(error) {
    ...
    failed = true;
    console.error(error);
    // The ONE added line: tools/ci/viewer_smoke.mjs and phase 60's
    // viewer-check.yml read this attribute, and it must be set BEFORE the
    // #status line renders so a failure can never look like silence.
    document.documentElement.setAttribute(
      'data-replay-error', error.message || String(error));
    tell('error', error.message || String(error));
  ...
  function createCore(config) {
    ...
    function onWorkerMessage(event) {
      ...
      try {
        if (message.type === 'text') {
          ...
          // one frame after the attribute the smoke reads is set.
    ...
    function start() {
      ...
      tell('loading');
      try {
        worker = new Worker(workerUrl, { name: 'coins-static-replay' });
  ...
  window.CoinsStaticReplay = {
    createCore: createCore,
    tell: tell
  };
  // The starter's page detects the static bundle by the presence of this
  // object; keep the inherited name as an alias so no page edit is needed.
  window.CtfStaticReplay = window.CoinsStaticReplay;
})();
